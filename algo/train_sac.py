"""Minimal SAC trainer for MuFlex using the main-branch environment API."""
...
class ResettableMuFlex(gym.Env):
    ...
    def __init__(
        ...
        reward_mode: str = "demand_limiting_tunable",
        save_results: bool = False,
        include_hour: bool = True,
        print_step_info: bool = False,
        max_total_hvac_power: float = 9_000.0,
        ...
        self.max_total_hvac_power = float(max_total_hvac_power)
        self.hvac_weight = float(hvac_weight)
        self.temp_weight = float(temp_weight)
        self.max_power_weight = float(max_power_weight)
    ...
    def _make_inner_env(self):
        ...
        # Keep core env untouched: inject reward parameters only for scripts that use them.
        env.max_total_hvac_power = self.max_total_hvac_power
        env.hvac_weight = self.hvac_weight
        env.temp_weight = self.temp_weight
        env.max_power_weight = self.max_power_weight
...
IDE_RUN_DEFAULTS = {
    ...
    "fmu_path": r"models\small_office\small_baseline_v1.fmu",
    "io_type": "OfficeS",
    "sim_days": 1,
    "start_date": 201,
    "step_size": 900,
    ...
    "run_name": "sac_small_baseline_v2",
    "output_dir": "runs",
    "log_interval": 100,
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Minimal SAC training on one MuFlex FMU.")
    ...
    parser.add_argument("--max-total-hvac-power", type=float, default=IDE_RUN_DEFAULTS["max_total_hvac_power"])
    parser.add_argument("--hvac-weight", type=float, default=IDE_RUN_DEFAULTS["hvac_weight"])
    parser.add_argument("--temp-weight", type=float, default=IDE_RUN_DEFAULTS["temp_weight"])
    parser.add_argument("--max-power-weight", type=float, default=IDE_RUN_DEFAULTS["max_power_weight"])
    ...
    parser.add_argument("--buffer-size", type=int, default=IDE_RUN_DEFAULTS["buffer_size"])
    parser.add_argument("--learning-starts", type=int, default=IDE_RUN_DEFAULTS["learning_starts"])
    parser.add_argument("--batch-size", type=int, default=IDE_RUN_DEFAULTS["batch_size"])
    parser.add_argument("--tau", type=float, default=IDE_RUN_DEFAULTS["tau"])
    parser.add_argument("--gamma", type=float, default=IDE_RUN_DEFAULTS["gamma"])
    parser.add_argument("--train-freq", type=int, default=IDE_RUN_DEFAULTS["train_freq"])
    parser.add_argument("--gradient-steps", type=int, default=IDE_RUN_DEFAULTS["gradient_steps"])
    parser.add_argument("--ent-coef", default=IDE_RUN_DEFAULTS["ent_coef"])
    ...
    parser.add_argument("--log-interval", type=int, default=IDE_RUN_DEFAULTS["log_interval"])
...
def build_run_dir(project_root: Path, output_dir: str, run_name: str | None) -> Path:
    ...
    final_run_name = run_name or f"sac_run_{timestamp}"
    return project_root / output_dir / final_run_name


def build_env(args: argparse.Namespace, fmu_configs: list[dict]) -> Monitor:
    env = ResettableMuFlex(
        ...
        max_total_hvac_power=args.max_total_hvac_power,
        hvac_weight=args.hvac_weight,
        temp_weight=args.temp_weight,
        max_power_weight=args.max_power_weight,
    )
    return Monitor(env)
...
def main() -> int:
    ...
    try:
        try:
            train_env = build_env(args, fmu_configs)
        ...
        if args.skip_eval:
            ...
            eval_env = build_env(args, fmu_configs)
